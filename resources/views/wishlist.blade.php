                ...
                <div class="cart-table__wrapper">
                    <table class="cart-table">
                        ...
                        @foreach ($cartItems as $wishlistItem)
                            @php
                                $product         = $wishlistItem->model; // may be null
                                $imageFromModel  = $product?->image;
                                $imageFromOption = data_get($wishlistItem->options, 'image');
                                $imageFile       = $imageFromModel ?: $imageFromOption;
                                $imageUrl        = $imageFile
                                    ? asset('uploads/products/thumbnails/'.$imageFile)
                                    : asset('images/placeholder.png'); // ensure this file exists
                            @endphp
                            <tr>
                                <td>
                                    <div class="shopping-cart__product-item">
                                        <img loading="lazy" src="{{ $imageUrl }}" width="120" height="120" alt="">
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="del-action d-flex gap-2">
                                        {{-- Move to Cart --}}
                                        <form method="POST" action="{{ route('wishlist.move.to.cart', $wishlistItem->rowId) }}">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-warning">Move to Cart</button>
                                        </form>

                                        {{-- Remove --}}
                                        <form method="POST" action="{{ route('wishlist.remove', $wishlistItem->rowId) }}">
                                            @csrf
                                            @method('DELETE')
